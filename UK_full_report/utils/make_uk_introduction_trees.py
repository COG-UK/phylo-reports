...
def find_tallest_tree(input_dir):
    ...
    for tree_dir in os.listdir(input_dir):
        if tree_dir != ".DS_Store":
            for r,d,f in os.walk(input_dir + tree_dir):
                for fn in f:
                    if fn.endswith("tree"):
                        treename = fn
                        num_taxa = 0
                        ...
                        if num_taxa > 1:
                            ...
                            for k in tree.Objects:
                                if k.branchType == 'leaf':
                                    if k.traits["country_uk"]=="True": #this will change
...
def display_name(tree, taxon_dict):
    for k in tree.Objects:
        if k.branchType == 'leaf':
            info = str(k.name).split("/")
            #details = info[0].split("/")
            name = k.name
            # if name == "":
            #     name = info[1]
            date = info[-1]
            county = taxon_dict[name].adm2.replace("_", " ")
            #county = info[-2].replace("_"," ")
            #county = info[0]
            if county == "":
                county = info[0]
            if "country" in k.traits: 
                if k.traits["country"] in ["Wales","Scotland","England","Northern_Ireland","WALES"]:
...
def sort_trees(tree_order, lineage_dict):
    d_list = []
    for i in tree_order:
        d = list(lineage_dict[i].acctrans_designations)[0]
        d_list.append(d)
    return d_list
...
def make_all_of_the_trees(input_dir, df_counts, tree_order, lineage_dict, taxon_dict, min_uk_taxa=3):
    ...
    lst = sort_trees(tree_order, lineage_dict)

    for fn in lst:
        intro = fn
        glob_lin = intro.split("_")[0]
        
        #intro = fn.split(".")[0]
        #glob_lin = intro.split("_")[1]
        #glob_lin = lineage_dict[intro].overall_lineage
        tree_dir = glob_lin
        
       
        #acctrans = str(list(lineage_dict[intro].acctrans_designations)[0])
        treename = "acc_lineage_" + intro + ".tree"
        
        
        num_taxa = 0
        intro_name = ""
        with open(input_dir + tree_dir + "/" + treename,"r") as f:
            for l in f:
                l = l.rstrip("\n")
                if l.startswith("    DIMENSIONS NTAX"):
                    num_taxa = int(l.rstrip(";").split("=")[1])
        ...
        if num_taxa > 1:
            tree = bt.loadNewick(input_dir + tree_dir + "/" + treename, absoluteTime=False)
            tips = []
            uk_tips = []
            for k in tree.Objects:
                if k.branchType == 'leaf':
            ...
            if len(uk_tips)>min_uk_taxa:
                ...
                try:
                    make_scaled_tree_without_legend(tree,len(tips),colour_dict, tallest_height, intro, taxon_dict)  
                    make_pie(intro, df_counts)    
